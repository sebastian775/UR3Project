...
{
    public class CoorPublisher : UnityPublisher<MessageTypes.Geometry.PoseArray>
    {
        ...
        public void coordinates()
        {
            ...
            for (int j = 0; j < numer; j++)
            {
                ...
                message.poses[j].orientation.w = float.Parse(GameObject.Find("Coor"+(j)+"/VOriW").GetComponent<TMP_Text>().text);
            }
            UpdateMessage();
        }
