...
class TrajectoryClient:
    ...
    def send_joint_trajectory(self,listaP):
        """Creates a trajectory and sends it using the selected action server"""

        # make sure the correct controller is loaded and activated
        self.switch_controller(JOINT_TRAJECTORY_CONTROLLERS[2])
        trajectory_client = actionlib.SimpleActionClient(
            "{}/follow_joint_trajectory".format(JOINT_TRAJECTORY_CONTROLLERS[2]),
        ...
        rospy.loginfo("Executing trajectory using the {}".format(JOINT_TRAJECTORY_CONTROLLERS[2]))
    ...
    def send_cartesian_trajectory(self, listaP):
        #self.choose_controller(self)
        """Creates a Cartesian trajectory and sends it using the selected action server"""
        self.switch_controller( CARTESIAN_TRAJECTORY_CONTROLLERS[1])

        # make sure the correct controller is loaded and activated
        goal = FollowCartesianTrajectoryGoal()
        trajectory_client = actionlib.SimpleActionClient(
            "{}/follow_cartesian_trajectory".format( CARTESIAN_TRAJECTORY_CONTROLLERS[1]),
            FollowCartesianTrajectoryAction,
        )
        ...
        for i in range(4,(len(pose_list)+3)):
        ...
        rospy.loginfo(
            "Executing trajectory using the {}".format( CARTESIAN_TRAJECTORY_CONTROLLERS[1])
        )
        trajectory_client.send_goal(goal)
        trajectory_client.wait_for_result()
    ...
    def send_trajectory(self, listaP):
        #self.choose_controller(self)
        """Creates a Cartesian trajectory and sends it using the selected action server"""
        self.switch_controller( CARTESIAN_TRAJECTORY_CONTROLLERS[1])
